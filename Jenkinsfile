  ...
  stages {
    ...
    stage("Set ENV Variables base"){
      steps{
        script{
          env.EXIT_STATUS = ''
          env.LS_RELEASE = sh(
            script: '''docker run --rm alexeiled/skopeo sh -c 'skopeo inspect docker://docker.io/'${DOCKERHUB_IMAGE}':develop 2>/dev/null' | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-ls' || : ''',
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' git ls-remote https://git.tt-rss.org/fox/tt-rss.git HEAD | cut -c1-8 ''',
            returnStdout: true).trim()
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=develop -v ${TEMPDIR}:/ansible/jenkins linuxserver/jenkins-builder:latest 
              docker pull linuxserver/doc-builder:latest
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=develop -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || \
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f develop
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-develop"
          ...
          sh "docker push ${IMAGE}:arm32v7-develop"
          sh "docker push ${IMAGE}:arm64v8-develop"
          sh "docker manifest push --purge ${IMAGE}:develop || :"
          sh "docker manifest create ${IMAGE}:develop ${IMAGE}:amd64-develop ${IMAGE}:arm32v7-develop ${IMAGE}:arm64v8-develop"
          sh "docker manifest annotate ${IMAGE}:develop ${IMAGE}:arm32v7-develop --os linux --arch arm"
          ...
          sh '''docker rmi \
                ...
                ${IMAGE}:amd64-develop \
                ${IMAGE}:arm32v7-${META_TAG} \
                ${IMAGE}:arm32v7-develop \
                ${IMAGE}:arm64v8-${META_TAG} \
                ${IMAGE}:arm64v8-develop \
                lsiodev/buildcache:arm32v7-${COMMIT_SHA}-${BUILD_NUMBER} \
                lsiodev/buildcache:arm64v8-${COMMIT_SHA}-${BUILD_NUMBER} || :'''
        }
      }
