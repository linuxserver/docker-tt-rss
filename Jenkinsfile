        ...
        withCredentials([
          ...
          sh '''#! /bin/bash
                ...
                for LEGACYIMAGE in "${QUAYIMAGE}" "${GITHUBIMAGE}"; do
                  ...
                  docker tag ${LEGACYIMAGE}:amd64-${META_TAG} ${LEGACYIMAGE}:develop
                  docker tag ${LEGACYIMAGE}:amd64-${META_TAG} ${LEGACYIMAGE}:${META_TAG}
                  docker tag ${LEGACYIMAGE}:arm32v7-${META_TAG} ${LEGACYIMAGE}:arm32v7-develop
                  docker tag ${LEGACYIMAGE}:arm64v8-${META_TAG} ${LEGACYIMAGE}:arm64v8-develop
                  docker push ${LEGACYIMAGE}:amd64-${META_TAG}
                  docker push ${LEGACYIMAGE}:arm32v7-${META_TAG}
                  docker push ${LEGACYIMAGE}:arm64v8-${META_TAG}
                  docker push ${LEGACYIMAGE}:develop
                  docker push ${LEGACYIMAGE}:${META_TAG}
                  docker push ${LEGACYIMAGE}:arm32v7-develop
                  docker push ${LEGACYIMAGE}:arm64v8-develop
                done
             '''
          sh '''#! /bin/bash
                for DELETEIMAGE in "${QUAYIMAGE}" "${GITHUBIMAGE}" "${GITLABIMAGE}" "${IMAGE}"; do
